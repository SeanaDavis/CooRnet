...
build_coord_graph <- function(ct_shares.df, coordinated_shares, percentile_edge_weight=0.90, timestamps=FALSE) {
  ...
  highly_connected_coordinated_entities <- highly_connected_coordinated_entities %>%
    mutate(account.url_expanded = ifelse(grepl("/groups/", highly_connected_coordinated_entities$name) & highly_connected_coordinated_entities$account.handle=="NA", name, longurl::expand_urls(name, warn = F, agent = "cooRnet-r-package", .progress = TRUE)$expanded_url),
